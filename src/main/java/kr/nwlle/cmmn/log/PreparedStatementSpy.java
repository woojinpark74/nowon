...
public class PreparedStatementSpy extends StatementSpy implements PreparedStatement
{
  ...
  private static final boolean SHOW_TYPE = false;
  ...
  protected void argTraceSet(int i, String typeHelper, Object arg)
  {
    ...
    startNum--;  // make the index 0 based
    synchronized (argTrace)
    {
      // if an object is being inserted out of sequence, fill up missing values with null...
      while (startNum >= argTrace.size())
      {
        argTrace.add(argTrace.size(), null);
      }
      if (!SHOW_TYPE || typeHelper == null)
      {
        argTrace.set(startNum, tracedArg);
      }
      else
      {
        argTrace.set(startNum, typeHelper + tracedArg);
      }
    }
  }
  ...
  protected String dumpedSql()
  {
    ...
    int lastPos = 0;
    int pos = sql.indexOf('?', lastPos);  // find position of first question mark
    int argIdx = 0;
    String arg;
    ...
    while (pos != -1)
    {
      ...
      dumpSql.append(sql.substring(lastPos, pos));  // dump segment of sql up to question mark.
      lastPos = pos + 1;
