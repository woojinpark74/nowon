...
public class Slf4jSpyLogDelegator implements SpyLogDelegator
{
   ...
   * Create a SpyLogDelegator specific to the Simple Logging Facade for Java (slf4j).
    ...
    {
        String classType = spy.getClassType();
        Logger logger = ResultSetSpy.CLASS_TYPE.equals(classType) ?
   ...
   * based on the various DumpSqlXXXXXX flags.
    ...
    private boolean shouldSqlBeLogged(String sql)
    {
        ...
        mysql = mysql.substring(0, 6).toLowerCase();
        return
          (DriverSpy.dumpSqlSelect && "select".equals(mysql)) ||
          (DriverSpy.dumpSqlInsert && "insert".equals(mysql)) ||
          (DriverSpy.dumpSqlUpdate && "update".equals(mysql)) ||
          (DriverSpy.dumpSqlDelete && "delete".equals(mysql)) ||
          (DriverSpy.dumpSqlCreate && "create".equals(mysql));
    }
    ...
    public String sqlOccured(Spy spy, String methodCall, String sql)
    {
        ...
        if (!DriverSpy.dumpSqlFilteringOn || shouldSqlBeLogged(strSQL))
        {
        	strSQL = processSql(strSQL);

            if (sqlOnlyLogger.isDebugEnabled())
            {
                sqlOnlyLogger.debug(getDebugInfo() + nl + spy.getConnectionNumber() + ". " + strSQL);
            }
            else if (sqlOnlyLogger.isInfoEnabled())
            {
                sqlOnlyLogger.info(strSQL);
            }
            return strSQL;
        }
    ...
    private String processSql(String sql)
    {
        ...
        else
        {
            ...
            StringTokenizer st = new StringTokenizer(mysql);
            String token;
            int linelength = 0;
